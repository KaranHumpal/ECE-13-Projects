...
int main(void)
{
    ...
    printf("\n\nWelcome to khumpal's sort.c, compiled on %s %s.\n", __DATE__, __TIME__);
    ...
    __asm("nop"); //put a breakpoint on this line, and record the stopwatch time 104.671ms
    ...
    __asm("nop"); //put a breakpoint on this line, and record the stopwatch time 46.543ms
    LinkedListPrint(listToSort);
...
ListItem *SelectionSort(ListItem* list)
{
   //YOUR CODE GOES HERE!
    ListItem *FU;
    FU = LinkedListGetFirst(list);//pointer to first item
    
    while(FU -> nextItem != NULL){ //While FU is not the tail
        ListItem *S;
        S = FU -> nextItem; 
        while(S-> nextItem){
            int comp = strcmp(FU ->data, S ->data);
            if(comp>0){
        ...
        if (S->nextItem == NULL) {
            int comp2 = strcmp(FU->data, S->data);
            if (comp2 > 0){
                LinkedListSwapData(FU, S);
                }
            }         
...
ListItem *InsertionSort(ListItem* list)
{
   //YOUR CODE GOES HERE!
    ...
    while(FS->previousItem != NULL){
        ListItem *LU;
        LU = FS->previousItem;
        int comp2 = strcmp(LU->data, FS->data);
        if(comp2<0){
            FS=LU;
        }else{
                ListItem *S;
                ...
                while(S->nextItem != NULL){
                    int comp3 = strcmp(S->nextItem->data,LU->data);
                    if(comp3>0){
                        break;
                    }
                    else{
                ...
                char *remove;
                remove = LinkedListRemove(LU);
                LinkedListCreateAfter(S, remove);
                }
            }
    return FS;
        }
