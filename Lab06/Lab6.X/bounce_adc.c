...
// Enable interrupts for the ADC
     ...
     **************************************************************************************************/
  printf("Welcome to khumpal's lab6 part3 (bounce_adc).  Compiled on %s %s.\n",__TIME__,__DATE__);
  OledInit();
  char message[60];
  int percentage = (((float) adcResult.voltage / 1023) * 100);
  sprintf(message, "Potentiometer Value: \n %d \n %d%%", adcResult.voltage, percentage);
...
void __ISR(_ADC_VECTOR, ipl2auto) AdcHandler(void)
{
    ...
    IFS1bits.AD1IF = 0;
        if (abs(adcResult.voltage - average) > 1) {
        adcResult.voltage = average;
        adcResult.event = 1;
    }
